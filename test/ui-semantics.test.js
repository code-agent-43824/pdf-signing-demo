...
test('UI separates integrity, trust and qualified status without false success claim', () => {
  ...
  assert.doesNotMatch(html, /успешно подписан[а-яё\s]*квалифицированной электронной подписью/i);
  ...
  assert.match(previewUi, /Цепочка доверия, срок, отзыв и назначение ключа не проверялись/);
  assert.match(
    cryptoProAdapter,
...
test('UI requires an explicit, digest-bound signing confirmation', () => {
  ...
  assert.match(
    orchestrator,
    /await confirm\(\{[\s\S]*documentName:[\s\S]*documentDigest,[\s\S]*certificate: context\.certificate/,
  );
  assert.ok(orchestrator.indexOf('await confirm(') < orchestrator.indexOf('apiClient.prepare('));
});
...
test('certificate usability and Rutoken PIN lifecycle are fail-closed', () => {
  ...
  assert.match(app, /querySelectorAll\('\[data-sensitive-input\]'\)[\s\S]*input\.value = ''/);
...
test('saved crypto stack beats the markup default and leaving CryptoPro stops its wait', () => {
  ...
  const switchStack =
    app.match(/async function switchCryptoStack\(mode\) \{[\s\S]*?\n\}/)?.[0] || '';
