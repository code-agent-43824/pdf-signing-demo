(function attachRutokenAdapter(root) {
  ...
  function isAlreadyLoggedInError(error, plugin) {
    ...
    return (
      message.includes('ALREADY_LOGGED_IN')
    ...
    for (const deviceId of deviceIds || []) {
      ...
      for (const category of categories) {
        ...
        for (const certId of certIds || []) {
          ...
          const keyUsageAllowed =
            keyUsageSource === undefined
            || keyUsageSource === null
            || normalizedKeyUsages.some(
              (usage) =>
  ...
  function createEnvironment({
    ...
    function attachTokenMonitor(plugin) {
      const monitorSource =
        plugin?.originalObject && typeof plugin.originalObject.tokenMonitor === 'function'
      ...
      monitorSource.tokenMonitor((type, slotId) => {
        ...
        diagnostic(
          ...
          type === 'connected' ? 'обновление…' : 'не вставлен',
        );
    ...
    async function initialize() {
      ...
      try {
        ...
        if (error?.unresponsiveComponent)
          diagnostic(error.unresponsiveComponent, 'error', 'не отвечает');
        throw error;
      }
    }
