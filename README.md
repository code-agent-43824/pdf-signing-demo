...
(runtime-пакеты и инструменты разработки), выполняет `npm ci`, проверяет lint
и форматирование JS и Python, воспроизводит фикстуры, запускает полный набор
тестов, `npm audit` production-зависимостей и проверяет закоммиченный SBOM.
Перед коммитом код приводится к нужному формату командой `npm run format`.
...
Node — системный бинарник. `.venv` нужен Python-воркерам и Ruff: сервер и
тесты запускают `python3` из `PATH`, а `npm run lint` — `ruff`.
